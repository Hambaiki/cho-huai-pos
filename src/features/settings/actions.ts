...
async function requireStoreManager(
  ...
  if (!membership || !["owner", "manager"].includes(membership.role)) {
    return {
      data: null,
      error: "Only owners or managers can manage categories.",
    };
  }
...
export async function getStoreInviteCodes(
  storeId: string,
  ...
  const { data } = await supabase
    .from("invite_codes")
    .select(
      "id, code, role, used_count, max_uses, expires_at, note, is_revoked",
    )
    .eq("store_id", storeId)
    .eq("is_revoked", false)
...
export async function getQrChannels(storeId: string) {
  const supabase = await createClient();
  const { data } = await supabase
    .from("qr_channels")
