...
export function ProductModal({
  ...
  const [state, formAction, isPending] = useActionState(
    async (_prevState: Result | null, formData: FormData) => {
      void _prevState;
  ...
  const [removeImageState, removeImageFormAction, isRemovingImagePending] =
    useActionState(async (_prevState: Result | null) => {
      void _prevState;

      if (!product) {
        return { ok: false, error: "Product not found." } satisfies Result;
      }
