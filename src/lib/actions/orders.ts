...
export async function createOrderAction(
  ...
  if (isBnpl) {
    ...
    if (account.status !== "active") {
      return { data: null, error: "Only active BNPL accounts can be used for checkout." };
    }

    if (Number(account.balance_due) + total > Number(account.credit_limit)) {
      return { data: null, error: "This BNPL account does not have enough available credit." };
    }
  ...
  const { data: order, error: orderError } = await supabase
    ...
    .insert({
      ...
      change_amount:
        !isBnpl && parsed.data.amountTendered && parsed.data.amountTendered > total
  ...
  if (isBnpl && bnplAccount) {
    ...
    if (installmentError || !installment) {
      ...
      return {
        data: null,
        error: installmentError?.message ?? "Failed to create the BNPL installment.",
      };
    }
    ...
    if (bnplBalanceError) {
      await supabase.from("bnpl_installments").delete().eq("id", bnplInstallmentId);
      await rollbackOrder();
