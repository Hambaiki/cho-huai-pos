...
> **Last updated:**
> **Rev 1:** Draft created
> **Rev 2:** Added QR payment implementation
> **Rev 3:** Renamed to CHO-HUAI POS, Thai context, per-store currency settings
...
> **Rev 5:** Added invite code gating for new signups and super-admin panel (`/admin`)
> **Rev 6:** Owner-provisioned staff accounts via `supabaseAdmin.auth.admin.createUser()`; `is_provisioned` flag on `profiles`; replaced store invite email flow with direct credential creation
> **Rev 7:** Per-store provisioned account limit and per-account store limit — sitewide defaults with per-store/per-account overrides configurable by super-admin; hard block on limit breach
...
7. [Invite Code Gating & Super-Admin](#7-invite-code-gating--super-admin)
8. [Authentication & Multi-Tenancy](#8-authentication--multi-tenancy)
9. [Role-Based Access Control](#9-role-based-access-control)
10. [POS Terminal](#10-pos-terminal)
11. [Inventory Management](#11-inventory-management)
12. [BNPL System](#12-bnpl-system)
13. [QR Transfer Payment](#13-qr-transfer-payment)
14. [Team Management](#14-team-management)
15. [Reports](#15-reports)
16. [App Router — Pages & Routes](#16-app-router--pages--routes)
17. [API Routes (Server Actions / Route Handlers)](#17-api-routes-server-actions--route-handlers)
...
│       ├── limits.ts                    # resolveStaffLimit, resolveStoreLimit
│       ├── receipt.ts
...
create table profiles (
  ...
  -- is_provisioned = true: account was created by a store owner on behalf of a staff member.
  -- These accounts cannot create stores. They go directly to their store dashboard on login.
  -- is_provisioned = false: account was self-registered with a super-admin invite code (store owner).
  store_limit_override  integer,
  -- null = use sitewide default from site_settings ('default_store_limit_per_account').
  -- Set by super-admin to raise or lower the max number of stores this account can own.
...
create table stores (
  ...
  -- null = use sitewide default from site_settings ('default_staff_limit_per_store').
  -- Set by super-admin to raise or lower the max provisioned accounts for this specific store.
  created_at          timestamptz default now()
);
```
...
See §7 for the full schema. Managed exclusively by the super-admin via `/admin/invite-codes`.

### 5.15 site_settings

See §7 for the full schema. Key-value store for sitewide flags managed via `/admin/settings`.

### 5.16 invites

> **Note:** The old email-based invite token flow (store owner sends link → employee signs up) has been replaced by owner-provisioned accounts (§8, Path B). The `invites` table below is **retained only for potential future use** (e.g. inviting an existing app user to join an additional store). It is not part of the initial implementation.
...
## 7. Invite Code Gating & Super-Admin

### Overview

CHO-HUAI POS is a closed-access service. New users cannot sign up without a valid invite code issued by the project owner (super-admin). This keeps the user base controlled at launch and prevents unsolicited registrations.

The project owner has a dedicated super-admin account identified by a `is_super_admin` flag on their `profiles` row. Super-admin access is managed entirely through a hidden `/admin` route, protected at the middleware level.
...
create table invite_codes (
  id            uuid primary key default gen_random_uuid(),
  code          text not null unique,             -- short human-readable code, e.g. "CHOPH-2024-A3"
  max_uses      integer not null default 1,       -- how many accounts can use this code
  used_count    integer not null default 0,       -- incremented on each successful signup
  created_by    uuid references auth.users(id),   -- always the super-admin
  note          text,                             -- internal label, e.g. "Batch for March launch"
...
```
User visits /signup
→ Enters email, password, and invite code
→ Client submits to signUp Server Action
→ Server validates invite code:
    - Exists? Not revoked? Not expired? used_count < max_uses?
    - If invalid → return { error: "Invalid or expired invite code" }
→ Create Supabase Auth user
→ Create profiles row
→ Increment invite_codes.used_count
→ Redirect to store creation (onboarding)
```

The invite code check happens **inside the same Server Action** as account creation, wrapped in a Postgres transaction so a failed signup never increments the `used_count`.
...
export async function signUp(input: SignUpInput): Promise<ActionResult<void>> {
  ...
  const { data: code, error: codeError } = await supabaseAdmin
    .from("invite_codes")
    .select("*")
    .eq("code", input.invite_code.toUpperCase().trim())
    .single();

  if (codeError || !code) return { data: null, error: "Invalid invite code." };
  if (code.is_revoked)
    return { data: null, error: "This invite code has been revoked." };
  if (code.expires_at && new Date(code.expires_at) < new Date())
    return { data: null, error: "This invite code has expired." };
  ...
  if (authError) throw new Error(authError.message); // fatal
...
A hidden section of the same Next.js app, protected by middleware. Only accounts with `profiles.is_super_admin = true` can access it. Any other authenticated user hitting `/admin` is redirected to `/dashboard`.
...
if (pathname.startsWith("/admin")) {
  const isSuperAdmin = await checkSuperAdmin(supabase); // reads profiles.is_super_admin
  if (!isSuperAdmin)
...
| Feature                 | Description                                                                                                                                 |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **Invite codes**        | Generate new codes with a label, max uses, and optional expiry. Revoke any active code instantly. View usage count per code.                |
| **All stores**          | Paginated list of every store. View name, owner, member count, created date.                                                                |
| **All users**           | Paginated list of every `profiles` row. View email, display name, store memberships.                                                        |
| **Suspend a store**     | Toggle `stores.is_suspended = true`. Suspended stores show a locked screen to all members.                                                  |
| **Store staff limit**   | View and override `stores.staff_limit_override` per store. Falls back to sitewide default when null.                                        |
| **Account store limit** | View and override `profiles.store_limit_override` per user. Falls back to sitewide default when null.                                       |
| **Sitewide settings**   | `site_settings` key-value store: maintenance mode, announcement banner, `default_staff_limit_per_store`, `default_store_limit_per_account`. |

#### Supporting schema

```sql
-- Sitewide key-value settings
create table site_settings (
  key         text primary key,    -- e.g. 'maintenance_mode', 'announcement_text'
  value       text,
...
insert into site_settings (key, value) values
  ...
  ('announcement_text',             ''),
  ('default_staff_limit_per_store', '10'),   -- max provisioned accounts per store
  ('default_store_limit_per_account', '3');  -- max stores a single owner account can create
```
...
app/
  (admin)/
    layout.tsx          # Middleware-enforced super-admin guard + admin nav shell
    admin/
      page.tsx          # Overview dashboard (store count, user count, active codes)
      invite-codes/
        page.tsx        # List + generate + revoke codes
      stores/
        page.tsx        # All stores list
        [id]/page.tsx   # Store detail — suspend toggle + staff limit override
      users/
        page.tsx        # All users list
        [id]/page.tsx   # User detail — store limit override
      settings/
        page.tsx        # Sitewide settings (maintenance mode, announcement, default limits)
features/
  admin/
    components/         # InviteCodeTable, StoreListTable, SiteSettingsForm, LimitOverrideInput…
    actions.ts          # generateInviteCode, revokeInviteCode, suspendStore,
                        # setStoreLimitOverride, setAccountLimitOverride, updateSiteSetting
```

#### RLS for admin tables

Super-admin operations use the **service role key** (available only in Server Actions and Server Components, never exposed to the client). `invite_codes` and `site_settings` have RLS enabled but no user-facing policies — they are only accessible via service role:

```sql
alter table invite_codes enable row level security;
-- No user-facing policies; all access via service role in Server Actions
...
The `(store)/layout.tsx` Server Component checks `store.is_suspended` on every render. If true, it renders a fullscreen locked state instead of the normal layout — no further data is loaded.
...
Both limits are checked inside their respective Server Actions before any writes occur. The effective limit is resolved by preferring the per-entity override over the sitewide default.

```ts
// lib/utils/limits.ts

export async function resolveStaffLimit(storeId: string): Promise<number> {
  const [store, setting] = await Promise.all([
    supabaseAdmin
      .from("stores")
      .select("staff_limit_override")
      .eq("id", storeId)
      .single(),
    supabaseAdmin
      .from("site_settings")
      .select("value")
      .eq("key", "default_staff_limit_per_store")
      .single(),
  ]);
  return store.data?.staff_limit_override ?? Number(setting.data?.value ?? 10);
}

export async function resolveStoreLimit(userId: string): Promise<number> {
  ...
  return profile.data?.store_limit_override ?? Number(setting.data?.value ?? 3);
}
```

**Staff limit** — checked inside `provisionStaffAccount` before creating the auth user:

```ts
// features/team/actions.ts (inside provisionStaffAccount)
const [currentStaff, staffLimit] = await Promise.all([
  supabaseAdmin
    .from("store_members")
    .select("id", { count: "exact" })
    .eq("store_id", input.storeId)
    .neq("role", "owner"),
  resolveStaffLimit(input.storeId),
]);
if ((currentStaff.count ?? 0) >= staffLimit)
  return {
    data: null,
    error: `This store has reached its staff limit of ${staffLimit} accounts. Contact support to increase the limit.`,
  };
```

**Store limit** — checked inside `createStore` before inserting the store row:

```ts
// features/settings/actions.ts (inside createStore)
const [ownedStores, storeLimit] = await Promise.all([
  supabaseAdmin
    .from("stores")
    .select("id", { count: "exact" })
    .eq("owner_id", userId),
  resolveStoreLimit(userId),
]);
if ((ownedStores.count ?? 0) >= storeLimit)
  return {
    data: null,
    error: `You have reached your store limit of ${storeLimit}. Contact support to increase the limit.`,
  };
```

**Decision rule summary:**

| Limit              | Resolved from                                                                     | Checked in              | On breach                |
| ------------------ | --------------------------------------------------------------------------------- | ----------------------- | ------------------------ |
| Staff per store    | `stores.staff_limit_override` → `site_settings.default_staff_limit_per_store`     | `provisionStaffAccount` | Hard block, return error |
| Stores per account | `profiles.store_limit_override` → `site_settings.default_store_limit_per_account` | `createStore`           | Hard block, return error |
...
**Path A — Self-registration (store owner)**

1. User visits `/signup`, enters display name, email, password, and a **super-admin-issued invite code**.
2. Server Action validates the invite code (not revoked, not expired, `used_count < max_uses`).
3. Supabase Auth user is created; `profiles` row inserted with `is_provisioned = false`; `invite_codes.used_count` incremented — all in the same action.
4. User is redirected to the store creation onboarding flow.

**Path B — Owner-provisioned staff account**

1. Store owner fills in staff name, email, temporary password, and role in Team settings.
2. `provisionStaffAccount` Server Action creates the Supabase Auth user (email pre-confirmed), inserts `profiles` with `is_provisioned = true`, and inserts `store_members` with the assigned role.
3. Owner hands the credentials to the employee.
4. Employee visits `/login`, logs in immediately — no signup, no email verification step.
5. Middleware detects `is_provisioned = true` → store creation flow is blocked; user goes straight to `/dashboard`.

### Staff provisioning flow

Store owners create staff accounts directly from the Team settings page — no invite email, no signup flow for the employee. The owner enters the staff member's name, email, temporary password, and role. The Server Action creates the Supabase Auth account on their behalf using the service role key.

```
Owner fills in: display name, email, temporary password, role (manager/cashier/viewer)
→ provisionStaffAccount Server Action (service role):
    1. supabaseAdmin.auth.admin.createUser({ email, password, email_confirm: true })
    2. insert into profiles (id, display_name, is_provisioned = true)
    3. insert into store_members (store_id, user_id, role)
→ Owner hands credentials to the employee
→ Employee visits /login, logs in immediately — no signup, no email verification
→ Middleware detects is_provisioned = true → redirect to /dashboard (store creation blocked)
```
...
export async function provisionStaffAccount(
  ...
  // Validate caller is owner/manager of this store
  const callerRole = await getCallerRole(input.storeId);
  if (!callerRole || !["owner", "manager"].includes(callerRole))
    return { data: null, error: "Insufficient permissions." };

  // Create Supabase Auth user — skip email verification
  const { data: authUser, error: authError } =
    await supabaseAdmin.auth.admin.createUser({
      email: input.email,
  ...
  // Create profiles row flagged as provisioned
  const { error: profileError } = await supabaseAdmin.from("profiles").insert({
    id: authUser.user.id,
    display_name: input.displayName,
    is_provisioned: true,
  });
  if (profileError) throw new Error(profileError.message); // fatal

  // Assign store membership + role
  ...
  if (memberError) throw new Error(memberError.message); // fatal
...
`middleware.ts` handles three concerns on every request:

- Refreshes the Supabase session cookie
- Redirects unauthenticated users away from `(store)` and `/admin` routes
- Redirects `is_provisioned = true` users away from the store creation / onboarding flow
- Redirects non-super-admin users away from `/admin`
...
interface StoreContext {
  ...
  profile: {
    id: string;
    display_name: string;
    is_provisioned: boolean; // if true, store creation is blocked for this user
...
| Role      | Description                                                                                                |
| --------- | ---------------------------------------------------------------------------------------------------------- |
| `owner`   | Full access. The person who created the store. Cannot be removed. Always `is_provisioned = false`.         |
| `manager` | Operational access. Can manage inventory, BNPL, void orders, and provision cashier/viewer accounts.        |
| `cashier` | POS access only. Can ring up sales and optionally record BNPL payments. Typically `is_provisioned = true`. |
| `viewer`  | Read-only. Can view reports and orders but cannot make changes. Typically `is_provisioned = true`.         |
...
export async function setStoreLimitOverride(
  storeId: string,
  limit: number | null,
): Promise<void>;
// null = revert to sitewide default
...
// features/team/actions.ts
export async function provisionStaffAccount(
  data: ProvisionStaffInput,
): Promise<ActionResult<void>>;
export async function removeStaffAccount(
  userId: string,
  storeId: string,
): Promise<void>;
// removeStaffAccount removes from store_members; does NOT delete the auth user
...
### Staff provisioning flow (detailed)

The owner or manager adds staff directly from **Team settings** — no email links, no invite tokens. See §8 (Auth flow — Path B) for the full Server Action code.

Key rules:

- Only `owner` and `manager` roles can provision new staff accounts
- The provisioned account's role cannot exceed the caller's role — a manager cannot provision another manager (owner only)
- If a staff member's email already exists in `auth.users`, the action returns a validation error rather than throwing
- The owner should communicate the temporary password to the employee directly (in person or via their own channel); CHO-HUAI POS does not send it by email
- The employee can change their password after first login via `/settings/account`
...
| `/team/new`             | Yes  | owner/manager  | Client (provision staff form) |
| `/reports`              | Yes  | viewer         | Server                        |
| `/settings`             | Yes  | owner          | Client                        |
| `/settings/qr-channels` | Yes  | owner          | Client                        |
| `/admin`                | Yes  | super-admin    | Server                        |
| `/admin/invite-codes`   | Yes  | super-admin    | Server + Client               |
| `/admin/stores`         | Yes  | super-admin    | Server                        |
| `/admin/stores/[id]`    | Yes  | super-admin    | Server + Client               |
| `/admin/users`          | Yes  | super-admin    | Server                        |
| `/admin/settings`       | Yes  | super-admin    | Client                        |

---

## 17. API Routes (Server Actions / Route Handlers)
...
// features/team/actions.ts — see §8 for full provisionStaffAccount implementation
// export async function provisionStaffAccount(data: ProvisionStaffInput): Promise<ActionResult<void>>
...
features/
  ...
  team/
    ...
    actions.ts        # provisionStaffAccount, removeMember, updateMemberRole
  settings/
...
- [ ] Auth: sign up (with invite code gate), login, session middleware
- [ ] `invite_codes` table + super-admin seed account
- [ ] `site_settings` seeded with default limits (`default_staff_limit_per_store = 10`, `default_store_limit_per_account = 3`)
- [ ] `/admin` route group with middleware guard
...
- [ ] Staff provisioning form (`/team/new`) — name, email, temp password, role
- [ ] `provisionStaffAccount` Server Action (service role key)
- [ ] `is_provisioned` middleware guard — block store creation for provisioned accounts
- [ ] Role-based UI guards
- [ ] Member management page (list staff, remove member)
...
### Phase 6 — Super-Admin Panel (Week 9)

- [ ] Invite code generation + revocation UI (`/admin/invite-codes`)
- [ ] All stores list + suspend toggle + staff limit override (`/admin/stores`)
- [ ] All users list + store limit override (`/admin/users`)
- [ ] Sitewide settings — maintenance mode + announcement banner (`/admin/settings`)
- [ ] `StoreSuspendedScreen` component + suspension check in `(store)/layout.tsx`
